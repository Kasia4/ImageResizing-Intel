...
void loadImg(char *img, uint8_t **imgBPtr)
{
	...
	if (!(file = fopen(img, "rb"))
		|| fread(header, 1, HEADER_SIZE, file) != HEADER_SIZE)
	{
		exit(-1);
	}
			...
			else if (event.type == SDL_KEYDOWN)
			{
...

				{
                    switch (event.key.keysym.sym) {
                        case SDLK_LEFT: globWidth -= SCALING_STEP;
                            break;
                        case SDLK_RIGHT: globWidth += SCALING_STEP;
                            //++texRect.w;
                            break;
                        case SDLK_UP: globHeight -= SCALING_STEP;
                            //--texRect.h;
                            break;
                        case SDLK_DOWN: globHeight += SCALING_STEP;
                            //++texRect.h;
                            break;
                        default: break;
                    }
